...
const Notification = (props) => {
  ...
  const authenticateUser = (data) => {
    ...
    for (let i = 0; i < data.NotificationList?.length; i++) {
      ...
      d.setHours(H);
      d.setMinutes(M);
      if (+localStorage.getItem("notificationClickedTime") < d.getTime())
        count += 1;
  ...
  return (
    <React.Fragment>
        ...
        {isNotificationIconClicked && (
          <React.Fragment>
            <div
              ...
              onClick={() => setIsNotificationIconClicked(false)}
            ></div>
      ...
      {count && <span className={classes.notificationCount}>{count}</span>}
    </React.Fragment>
  );
};
