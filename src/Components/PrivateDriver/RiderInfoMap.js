...
const RiderInfoMap = ({ RIDER_DATA, driverPath }) => {
  ...
  function myInitMap() {
    const map = new window.google.maps.Map(document.getElementById("map"), {
      zoom: 11,
      center:
        RIDER_DATA[0]?.trip_status?.toLowerCase() !== "ended"
          ? {
              lat: +RIDER_DATA[0].pickup_latLng.split(",")[0],
              lng: +RIDER_DATA[0].pickup_latLng.split(",")[1],
            }
          : {
              lat: +RIDER_DATA[0].actual_pickup_latLng.split(",")[0],
              lng: +RIDER_DATA[0].actual_pickup_latLng.split(",")[1],
            },
    ...
    const marker1 = new window.google.maps.Marker({
      position:
        RIDER_DATA[0]?.trip_status?.toLowerCase() === "accepted"
          ? {
              lat: +RIDER_DATA[0].pickup_latLng.split(",")[0],
              lng: +RIDER_DATA[0].pickup_latLng.split(",")[1],
            }
      ...
      myTitle:
        RIDER_DATA[0]?.trip_status?.toLowerCase() === "accepted"
          ? RIDER_DATA[0].pickup_name
          : RIDER_DATA[0].actual_pickup_name,
      icon: startPoint,
      optimized: false,
    });
    const marker2 = new window.google.maps.Marker({
      position:
        RIDER_DATA[0]?.trip_status?.toLowerCase() !== "ended"
          ? {
              lat: +RIDER_DATA[0].dropoff_latLng.split(",")[0],
              lng: +RIDER_DATA[0].dropoff_latLng.split(",")[1],
            }
          : {
              lat: +RIDER_DATA[0].actual_dropOff_latLng.split(",")[0],
              lng: +RIDER_DATA[0].actual_dropOff_latLng.split(",")[1],
            },
      map,
      myTitle:
        RIDER_DATA[0]?.trip_status?.toLowerCase() !== "ended"
          ? RIDER_DATA[0].drop_name
          : RIDER_DATA[0].actual_dropOff_name,
      icon: endPoint,
      optimized: false,
    });
