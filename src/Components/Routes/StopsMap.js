...
const StopsMap = ({ stopData }) => {
  ...
  function myInitMap() {
    const map = new window.google.maps.Map(
      document.getElementById("main-stopsInfo-map"),
      {
        zoom: 12,
        center: {
          lat: stopData[Math.round(stopData.length / 2)].location.lat,
          lng: stopData[Math.round(stopData.length / 2)].location.lng,
        },
  ...
  return <div id="main-stopsInfo-map"></div>;
};
