...
const TimingsInfo = (props) => {
  ...
  const nextClickHandler = () => {
    if (
      ...
      setIsError("");
    } else setIsError("Please set the timing for atleast one day");
  };
    ...
    <React.Fragment>
        ...
        <div className="timingsInfo-container">
          {isDayTimeChange.dayname && (
            <div className="dayTimeChange-container">
              ...
              <p>
                Or to disable the {dayName}{" "}
                <span onClick={disableDayClickHandler}>click here</span>
              </p>
            </div>
          )}
          {isError && <p className="error">{isError}</p>}
          <label style={{ marginLeft: "6%", marginRight: "-3%" }}>
            {props.routeType} Time:
          </label>
          <input
            type="time"
            ref={timeInputRef}
          ...
          <div className="dayList-container" onClick={dayButtonClickHandler}>
            <div className="dayList-subcontainer">
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.monday && (
                    <span className="selected-time">{TIMINGS.monday}</span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.tuesday && (
                    <span className="selected-time">{TIMINGS.tuesday}</span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.wednesday && (
                    <span className="selected-time">{TIMINGS.wednesday}</span>
                  )}
                </div>
              </div>
            </div>
            <div className="dayList-subcontainer">
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.thursday && (
                    <span className="selected-time">{TIMINGS.thursday}</span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.friday && (
                    <span className="selected-time"> {TIMINGS.friday}</span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {TIMINGS.saturday && (
                    <span className="selected-time">{TIMINGS.saturday}</span>
                  )}
                </div>
              </div>
            </div>
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {TIMINGS.sunday && (
                  <span className="selected-time">{TIMINGS.sunday}</span>
                )}
              </div>
            </div>
          </div>
          <footer>
            <button className="button" onClick={backClickHandler}>
              Back
            </button>
      ...
      {isNextClicked && (
        <StudentsInfo
          ...
          backWizard={props.backWizard}
          nextWizard={props.nextWizard}
