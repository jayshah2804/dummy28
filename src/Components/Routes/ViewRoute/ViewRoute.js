...
const ViewRoute = (props) => {
    ...
    function myInitMap() {
        ...
        waypts = [];
        for (let i = (routeInfo.RouteType?.toLowerCase() === "dropping" ? 1 : 0); i < (routeInfo.RouteType?.toLowerCase() === "dropping" ? routeDetails?.length - 1 : routeDetails?.length - 2); i++) {
        ...
        routeDetails.forEach((position, i) => {
            if (
                ...
                myTitle = `<div id="infowindow-container" ><h3>${position.StopName.split(",")[0]
                    }`;
            }
