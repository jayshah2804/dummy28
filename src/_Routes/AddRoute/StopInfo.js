  ...
  function myInitMap() {
    ...
    const saveAssignButtonClickHandler = (e, id) => {
      ...
      if (staffUIds.size === 0) {
        ...
        waypts.splice(indexToBeSpliced - 1, 1);
      ...
      } else if (!isChecked && alreadyRouteCreateFlag == true) {
        let targetIndex = 0;
        for (let i = 0; i < STOP_DETAILS.length; i++) {
          if (
    ...
    filteredData.forEach((position, i) => {
      ...
      if (i === 0) {
        ...
        myTitle =
          `<div id=${i}>` +
          `<div id="infowindow-container" ><h4 id="infowindow-stopName">${
            position.stop
        ...
        myTitle += `</div><button id="infoWindowAssignButton">Assign</button></div >`;
        // }
      }
